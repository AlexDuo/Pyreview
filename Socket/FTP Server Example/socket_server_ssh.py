...
while True:
    ...
    while True:
        ...
        if os.path.isfile(filename):
            ...
            connection.send(str(file_size).encode('utf-8')) #send file size
            connection.recv(1024) #防止黏连, Wait for ack
            for line in f:
                m.update(line)
                connection.send(line)
            print('the md5 for the file is: ',m.hexdigest())
            f.close()
